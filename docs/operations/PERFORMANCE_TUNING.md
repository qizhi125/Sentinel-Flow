...
Sentinel-Flow 在设计之初就将高性能作为核心目标，通过无锁队列、零拷贝内存池、CPU 亲和性绑定等技术实现了高吞吐量下的稳定处理。本文档针对不同部署场景，提供系统级和应用级的性能调优建议，帮助用户在有限的硬件资源下获得最佳检测能力。
...
- **解析批处理**：`PacketPipeline` 中 `BATCH_RESERVE_SIZE` 默认 5000，适当增大可减少回调次数，但会增加内存占用。
