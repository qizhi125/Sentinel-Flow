...
`ObjectPool` 是 Sentinel-Flow 实现零拷贝数据流的核心基础组件。它管理固定大小的 `MemoryBlock` 对象，提供无锁的分配与回收机制，确保数据包在捕获、解析、展示全链路中避免频繁的 `new`/`delete` 操作，从而消除内存碎片并提升性能。
