...
public:
  ObjectPool() = default;
  explicit ObjectPool(std::size_t preallocate) { reserve(preallocate); }
  ...
  void release(T *obj) noexcept {
    if (!obj)
      return;
    std::lock_guard<std::mutex> lock(mutex_);
    auto *block = reinterpret_cast<PoolBlock *>(reinterpret_cast<char *>(obj) -
                                                offsetof(PoolBlock, storage));
