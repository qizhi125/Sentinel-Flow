...
struct BlockDeleter {
  void operator()(MemoryBlock *block) const {
    ...
    try {
      ...
      std::cerr << "[FATAL] BlockDeleter caught exception: " << e.what()
                << std::endl;
    } catch (...) {
...
struct RawPacket {
  ...
  RawPacket()
      : kernelTimestampNs(0), block(nullptr), linkLayerOffset(14),
        isTruncated(false) {}
};
...
struct ParsedPacket {
  ...
  std::string_view getHttpMethod() const {
    if (httpMethodLen == 0 || payloadData.empty())
      return {};
    return {reinterpret_cast<const char *>(payloadData.data()), httpMethodLen};
  }

  std::string_view getHttpUri() const {
    if (httpUriLen == 0 || httpUriOffset + httpUriLen > payloadData.size())
      return {};
    return {reinterpret_cast<const char *>(payloadData.data() + httpUriOffset),
            httpUriLen};
  }

  std::string_view getTlsSni() const {
    if (tlsSniLen == 0 || tlsSniOffset + tlsSniLen > payloadData.size())
      return {};
    return {reinterpret_cast<const char *>(payloadData.data() + tlsSniOffset),
            tlsSniLen};
  }
  ...
  std::string getSummary() const {
    ...
    if (srcPort == 0 && dstPort == 0) {
      ss << protocol << " " << getSrcIpStr() << " -> " << getDstIpStr()
         << " Len=" << length;
    } else {
