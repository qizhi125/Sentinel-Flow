...
public:
  // 定义回调契约替代 Qt 信号
  using BatchCallback =
      std::function<void(std::shared_ptr<std::vector<ParsedPacket>>)>;
  using ThreatCallback =
      std::function<void(const Alert &, const ParsedPacket &)>;
  ...
  // 依赖注入回调接口
  void setCallBack(BatchCallback batchCb, ThreatCallback threatCb,
