...
namespace sentinel::engine
{
    ...
    void PacketPipeline::setCallBack(BatchCallback batchCb, ThreatCallback threatCb, StatsCallback statsCb) {
    ...
    void PacketPipeline::run() {
        ...
        while (running.load(std::memory_order_acquire)) {
            try {
                ...
                if (SecurityEngine::instance().isIpBlocked(parsed.srcIp) || SecurityEngine::instance().isIpBlocked(parsed.dstIp)) {
                    continue;
                }
