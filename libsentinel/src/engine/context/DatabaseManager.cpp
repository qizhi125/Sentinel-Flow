...
bool DatabaseManager::init(const std::string &dbPathStr) {
  ...
  if (rc != SQLITE_OK) {
    std::cerr << "[Database] 数据库挂载失败: " << sqlite3_errmsg(db)
              << std::endl;
    return false;
  }
...
void DatabaseManager::saveRule(const IdsRule &rule) {
  ...
  const char *sql = "INSERT OR REPLACE INTO rules (id, enabled, protocol, "
                    "pattern, level, description) VALUES (?, ?, ?, ?, ?, ?);";
  sqlite3_stmt *stmt;
...
void DatabaseManager::saveRulesTransaction(
    const std::vector<IdsRule> &rulesList) {
  std::lock_guard<std::mutex> lock(dbMutex);
  if (!db || rulesList.empty())
    return;
...
std::vector<IdsRule> DatabaseManager::loadRules() {
  ...
  const char *sql = "SELECT id, enabled, protocol, pattern, level, description "
...
void DatabaseManager::saveBlacklist(const std::string &ip,
                                    const std::string &reason) {
  std::lock_guard<std::mutex> lock(dbMutex);
  if (!db)
    return;

  const char *sql = "INSERT OR REPLACE INTO blacklist (ip, reason, timestamp) "
                    "VALUES (?, ?, ?);";
  sqlite3_stmt *stmt;
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, 0) == SQLITE_OK) {
...
DatabaseManager::loadBlacklist() {
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, 0) == SQLITE_OK) {
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      ...
      bl.emplace_back(ip ? reinterpret_cast<const char *>(ip) : "",
...
void DatabaseManager::saveConfig(const std::string &key,
...
std::string DatabaseManager::loadConfig(const std::string &key,
...
void DatabaseManager::workerLoop() {
  while (running) {
    ...
    if (rc != SQLITE_OK) {
      std::cerr << "[DB] Begin transaction failed: " << (errMsg ? errMsg : "")
                << std::endl;
      sqlite3_free(errMsg);
      continue;
    }

    const char *sql = "INSERT INTO alerts (timestamp, rule_name, source_ip, "
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
      for (const auto &alert : batch) {
        ...
        ip_addr.s_addr = htonl(
            alert
                .sourceIp); // 注意：原始代码这里可能存的是主机字节序，假设是网络字节序则不需要
                            // htonl，这里保守先用 htonl 尝试
        inet_ntop(AF_INET, &ip_addr, ipBuf, INET_ADDRSTRLEN);

        sqlite3_bind_text(stmt, 3, ipBuf, -1, SQLITE_TRANSIENT);

        sqlite3_bind_int(stmt, 4, static_cast<int>(alert.level));
        sqlite3_bind_text(stmt, 5, alert.description.c_str(), -1,
                          SQLITE_STATIC);
    ...
    if (success) {
      ...
      if (rc != SQLITE_OK) {
        std::cerr << "[DB] Commit failed: " << (errMsg ? errMsg : "")
                  << std::endl;
      }
    } else {
      rc = sqlite3_exec(db, "ROLLBACK;", nullptr, nullptr, &errMsg);
      if (rc != SQLITE_OK) {
        std::cerr << "[DB] Rollback failed: " << (errMsg ? errMsg : "")
                  << std::endl;
      }
    }
    sqlite3_free(errMsg);
...
std::vector<Alert> DatabaseManager::loadRecentAlerts(int limit) {
  ...
  const char *sql = "SELECT timestamp, rule_name, source_ip, level, "
                    "description FROM alerts ORDER BY timestamp DESC LIMIT ?;";
  sqlite3_stmt *stmt;
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, 0) == SQLITE_OK) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      ...
      a.ruleName =
          ruleText ? reinterpret_cast<const char *>(ruleText) : "Unknown";

      const unsigned char *ipText = sqlite3_column_text(stmt, 2);
      if (ipText) {
        // 使用标准 C 的 inet_pton 进行字符串到 IPv4 转换
        struct in_addr ip_addr;
        if (inet_pton(AF_INET, reinterpret_cast<const char *>(ipText),
                      &ip_addr) == 1) {
          a.sourceIp = ntohl(ip_addr.s_addr); // 假定内部存储的是主机字节序
        } else {
          a.sourceIp = 0;
        }
      } else {
