...
SecurityEngine::~SecurityEngine() { forensicWorker_.stop(); }
  ...
  const std::vector<int> *hitRuleIds = nullptr;
  {
    ...
    if (hitRuleIds && !hitRuleIds->empty()) {
      ...
      if (rule.protocol == "ANY" || rule.protocol == packet.protocol) {
        ...
        std::string cacheKey =
            std::to_string(packet.srcIp) + "_" + std::to_string(rule.id);

        {
          std::shared_lock readLock(suppressionMutex);
          auto cacheIt = suppressionCache.find(cacheKey);
          if (cacheIt != suppressionCache.end() &&
              (currentMs - cacheIt->second) < SUPPRESSION_WINDOW_MS) {
            return std::nullopt;
          }
        }
        ...
        Alert alert = {static_cast<uint64_t>(time(nullptr)), rule.level,
                       packet.srcIp, rule.description,
