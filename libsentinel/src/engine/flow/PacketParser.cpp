    ...
    if (sv.starts_with("GET ") || sv.starts_with("POST ") ||
      ...
      if (!sv.starts_with("HTTP/")) {
        ...
        if (space1 != std::string_view::npos &&
            space1 < std::numeric_limits<uint16_t>::max()) {
...
std::optional<ParsedPacket> PacketParser::parse(const RawPacket &raw) {
  ...
  if (ipHeader->protocol == IPPROTO_TCP) {
    ...
    if (pkt.length > 0) {
      pkt.payloadData.assign(data + payloadOffset,
                             data + payloadOffset + pkt.length);
    ...
    if (length > payloadOffset) {
      uint32_t actualPayloadLen =
          std::min<uint32_t>(pkt.length, length - payloadOffset);
      pkt.payloadData.assign(data + payloadOffset,
                             data + payloadOffset + actualPayloadLen);
