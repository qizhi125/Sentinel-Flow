...
namespace sentinel::capture {
    ...
    bool EBPFCapture::setFilter(const std::string& filterExp) {
        std::cout << "[XDP] 驱动处于 AF_XDP 零拷贝模式。传统 BPF 表达式 [" << filterExp << "] 被忽略，过滤策略已由内核态 XDP 探针接管。" << std::endl;
        return true;
    }
    ...
    void EBPFCapture::start(const std::string& device) {
        if (running.load() || targetQueues.empty()) return;
        m_device = device;
        ...
        if (xsk_umem__create(&m_ctx->umem, m_ctx->umem_buffer, NUM_FRAMES * FRAME_SIZE,
                             &m_ctx->fq, &m_ctx->cq, &umem_cfg)) {
