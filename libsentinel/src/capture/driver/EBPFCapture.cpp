...
EBPFCapture::~EBPFCapture() { EBPFCapture::stop(); }
...
void EBPFCapture::init(
    const std::vector<sentinel::common::SPSCQueue<RawPacket> *> &queues) {
  targetQueues = queues;
}

bool EBPFCapture::setFilter(const std::string &filterExp) {
  std::cout << "[XDP] 驱动处于 AF_XDP 零拷贝模式。传统 BPF 表达式 ["
            << filterExp << "] 被忽略，过滤策略已由内核态 XDP 探针接管。"
            << std::endl;
  return true;
}
...
std::vector<std::string> EBPFCapture::getDeviceList() {
  ...
  if (if_nidxs != nullptr) {
    for (struct if_nameindex *intf = if_nidxs;
         intf->if_index != 0 || intf->if_name != nullptr; intf++) {
...
void EBPFCapture::start(const std::string &device) {
  ...
  if (libbpf_get_error(m_ctx->bpf_obj)) {
    std::cerr << "[XDP] 加载 " << objPath
              << " 失败，请确保编译输出目录存在此文件。" << std::endl;
    return;
  }

  if (bpf_object__load(m_ctx->bpf_obj)) {
    std::cerr << "[XDP] bpf_object__load 失败 (请检查是否有 root 权限)。"
              << std::endl;
    return;
  }

  struct bpf_program *prog =
      bpf_object__find_program_by_name(m_ctx->bpf_obj, "xdp_pass_or_drop");
  if (!prog) {
  ...
  if (!m_ctx->link) {
    std::cerr << "[XDP] 挂载 BPF 程序到接口 " << device << " 失败。"
              << std::endl;
    return;
  }

  m_ctx->xsks_map_fd =
      bpf_object__find_map_fd_by_name(m_ctx->bpf_obj, "xsks_map");

  if (posix_memalign(&m_ctx->umem_buffer, getpagesize(),
  ...
  if (xsk_umem__create(&m_ctx->umem, m_ctx->umem_buffer,
                       NUM_FRAMES * FRAME_SIZE, &m_ctx->fq, &m_ctx->cq,
                       &umem_cfg)) {
  ...
  if (xsk_socket__create(&m_ctx->xsk, m_device.c_str(), 0, m_ctx->umem,
                         &m_ctx->rx, &m_ctx->tx, &xsk_cfg)) {
  ...
  std::cout << "\033[1;32m[XDP] AF_XDP Zero-Copy 引擎在 " << device
            << " 上启动成功！\033[0m" << std::endl;
    ...
    if (fds[0].revents & POLLIN) {
      ...
      for (uint32_t i = 0; i < rcvd; i++) {
        const struct xdp_desc *desc =
            xsk_ring_cons__rx_desc(&m_ctx->rx, rx_idx++);
        uint64_t addr = desc->addr;
        uint32_t len = desc->len;

        uint8_t *pktData = static_cast<uint8_t *>(
            xsk_umem__get_data(m_ctx->umem_buffer, addr));
      ...
      if (xsk_ring_prod__needs_wakeup(&m_ctx->fq)) {
        recvfrom(xsk_socket__fd(m_ctx->xsk), nullptr, 0, MSG_DONTWAIT, nullptr,
                 nullptr);
      }
    }
  }
