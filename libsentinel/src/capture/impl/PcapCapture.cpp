...
void PcapCapture::init(
    const std::vector<sentinel::capture::PacketQueue *> &queues) {
  workerQueues = queues;
...
void PcapCapture::captureLoop() {
  ...
  while (running) {
    ...
    RawPacket raw;
    raw.kernelTimestampNs =
        (int64_t)header->ts.tv_sec * 1000000000L + header->ts.tv_usec * 1000L;
